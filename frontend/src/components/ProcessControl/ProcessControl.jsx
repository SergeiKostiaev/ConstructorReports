...
import { removeActiveProcess, updateProcessProgress } from '../features/reportsSlice';
...
const ProcessControl = () => {
    ...
    const { activeProcesses, reportStatuses } = useSelector(state => state.reports);
            ...
            try {
                ...
                const latestReports = reports
                    ...
                    .map(report => ({
                        ...
                        status: reportStatuses[report.id] || mapReportStatus(report),
    ...
    const handleAbortProcess = async (processId) => {
        try {
            const response = await fetch(`${API_URL}/api/report/${processId}/cancel`, {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('api_token'),
                },
            ...
            if (response.ok) {
                ...
                toast.warn(`Процесс #${processId} отменен`, { position: "top-right" });
            } else {
                toast.error('Ошибка при отмене процесса', { position: "top-right" });
            }
        } catch (error) {
            toast.error('Ошибка сети', { position: "top-right" });
        }
    };
        ...
        <div className={styles.processControl}>
                        ...
                        activeProcesses.map(process => (
                            <div key={process.id} className={styles.processItem}>
                                ...
                                <button
                                    ...
                                    onClick={() => handleAbortProcess(process.id)}
                                >
                                    Отменить
                                </button>
                            </div>
            ...
            <div className={styles.section}>
                ...
                <div className={styles.stats}>
                    ...
                    <div className={styles.statCard}>
                        <span className={styles.statValue}>{activeProcesses.length}</span>
